...
class GeminiService:
    ...
    def _resolve_store_name(
        self,
        create_if_missing: bool = False,
        display_name: str = "Internal Knowledge Base",
    ) -> Optional[str]:
        """FILE_SEARCH_STORE env 우선, 없으면 첫 번째 store 재사용, 필요 시 생성."""
        if self._store_name:
            return self._store_name

        env_store_name = os.getenv("FILE_SEARCH_STORE")
        if env_store_name:
            self._store_name = env_store_name
            return self._store_name
    ...
    def create_corpus(self, display_name: str) -> Optional[str]:
        """File Search Store 생성. 이미 존재하면 첫 번째 스토어 재사용."""
        try:
            existing_store_name = self._resolve_store_name(create_if_missing=False)
            if existing_store_name:
                print(f"Reusing File Search Store: {existing_store_name}")
                return existing_store_name

            created_store_name = self._resolve_store_name(
                create_if_missing=True, display_name=display_name
            )
            print(f"Created File Search Store: {created_store_name}")
            return created_store_name
    ...
    def upload_file_to_corpus(
        ...
        """
        파일을 File Search Store에 직접 업로드하고 인덱싱 작업을 시작.
        custom_metadata 예: [{"key": "category", "string_value": "marketing"}]
        """
        try:
            store_name = corpus_name or self._resolve_store_name(
            ...
            print(
                f"Uploading '{display_name}' (mime: {mime_type}) to File Search Store..."
            )
    ...
    def list_documents(self, corpus_name: Optional[str]):
    ...
    def ask_chatbot(self, query: str, model_id: str = FILE_SEARCH_MODEL):
        ...
        try:
            store_name = self._resolve_store_name(create_if_missing=False)
            if not store_name:
    ...
    def ask_chatbot_stream(
        self,
        query: str,
        model_id: str = FILE_SEARCH_MODEL,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if (
                    ...
                    print(
                        f"503 UNAVAILABLE, {wait}s 후 재시도 ({attempt + 1}/{max_retries - 1})..."
                    )
                    time.sleep(wait)
                    continue
                self._handle_error(e, model_id)
    ...
    def _handle_error(self, e, model_id):
        ...
        if "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
            raise Exception(
                "현재 사용 중인 Gemini 모델의 무료 호출 한도(Quota)를 초과했습니다. 잠시 후 다시 시도해 주세요."
            )
        elif "503" in error_msg or "UNAVAILABLE" in error_msg:
            raise Exception(
                "죄송합니다. 현재 Gemini 서버에 부하가 집중되고 있습니다. 잠시 후 다시 시도해 주세요."
            )
        elif "404" in error_msg or "NOT_FOUND" in error_msg:
            raise Exception(
                f"모델({model_id})을 찾을 수 없거나 File Search를 지원하지 않습니다."
            )
        raise e
